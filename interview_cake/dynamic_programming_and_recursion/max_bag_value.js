function maxDuffelBagValue(cakeTypes, capacity) {
  ...
  for (let i = 0; i <= capacity; i++) {
    ...
    cakeTypes.forEach((cakeType) => {
      weight = cakeType.weight;
      value = cakeType.value;
      if (weight == 0 && value > 0) {
        max = Infinity;
      } else if (weight <= i) {
        if (weight == 0) {
        ...
        let remainingCapacity = i - weight;
        let remainingCapacityValue = values[remainingCapacity];
        max = Math.max(remainingCapacityValue + value, max);
      }
    });
