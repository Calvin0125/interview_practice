...
function getExcludedColors(node) {
  let excludedColors = new Set();
  node.neighbors.forEach(neighbor => {
    if (neighbor.color != null) {
      excludedColors.add(neighbor.color);
    }
  });
...
function colorGraph(graph, colors) {
  ...
  graph.forEach(node => {
    if (node.neighbors.has(node)) {
      throw "Illegal coloring impossible for nodes with loops.";
    }
    
    let excludedColors = getExcludedColors(node)
    for (let i = 0; i < colors.length; i++) {
      if (!excludedColors.has(colors[i])) {
        node.color = colors[i];
        break;
      }
    }
