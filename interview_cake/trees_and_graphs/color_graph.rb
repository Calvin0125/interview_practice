...
def color_graph(graph, colors)
  ...
  graph.each do |node|
    if node.neighbors.include?(node)
      raise ArgumentError, "Legal coloring impossible for node with loop."
    end
    
    excluded_colors = node.neighbors.map(&:color).compact.to_set
    node.color = colors.find { |color| !excluded_colors.include?(color) }
  end
end
